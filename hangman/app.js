...
// Utility function to reset the words table
const resetWordsTable = async () => {
  try {
    console.log('Resetting words table...');
    await pool.query('TRUNCATE TABLE words'); // Clear the table
    console.log('Words table has been reset.');
  } catch (error) {
    console.error('Error resetting the words table:', error.message);
  }
};

// Reset the words table when the server starts
pool.connect().then(() => {
  resetWordsTable(); // Reset words table on connection
});
