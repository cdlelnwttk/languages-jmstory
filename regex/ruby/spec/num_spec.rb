...
RSpec.describe 'Regular Expression for octal literal' do
  ...
  let(:pattern) {/^0#{octal_digit}*$/}

  let(:should_pass) { ["0", "077772", "0111", "01234567"]}
  let(:should_fail) {["1", "08", "9"]}
  describe 'should pass' do
    it 'matches all expected strings' do
      should_pass.each do |str|
        expect(str).to match(pattern)
...
RSpec.describe 'Regular Expression for hex literal' do
  ...
  let (:pattern) {/^#{hex_start}#{hex_digit}+$/}
  let (:should_pass) {["0Xfffff", "0xfffff", "0x123abc", "0x1", "0xabcdef"]}
  let (:should_fail) {["fffff", "0x", "0X", "12345", "abcdef", "x133f", "0xzzzzz"]}
...
RSpec.describe 'Regular Expression for binary literal' do
  ...
  let(:pattern) {/^#{binary_start}#{binary_digit}+$/}

  let(:should_pass) { ["0B10101001", "0b1", "0B0", "0B000000000000000", "0b111111111111111111111111", "0b1010100101001"]}
  let(:should_fail) {["4", "0b", "0B", "10101001", "0b2", "0B34"]}
  describe 'should pass' do
    it 'matches all expected strings' do
      should_pass.each do |str|
        expect(str).to match(pattern)
